...
import { DEFAULTS, settingsFrom, startupSettings } from "../settings";

test("fresh installs start scanning with removal effects", () => {
  expect(DEFAULTS.enabled).toBe(true);
  expect(DEFAULTS.animate).toBe(true);
  expect(startupSettings(undefined)).toEqual(DEFAULTS);
});
...
test("one-time upgrade enables protection and effects without losing the API", () => {
  ...
  expect(startupSettings(saved)).toEqual({ ...saved, enabled: true, animate: true });
  expect(settingsFrom(saved)).toEqual(saved);
});
