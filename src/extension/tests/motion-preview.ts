...
const server = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
  ...
  if (path === "/effects.js") {
    const build = await Bun.build({ entrypoints: ["src/extension/effects.ts"], format: "esm", target: "browser" });
  ...
  if (path === "/") return new Response(Bun.file("src/extension/tests/motion-preview.html"), { headers: { "Cache-Control": "no-store" } });
