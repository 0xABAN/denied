...
After the last whole Short is successfully removed, the YouTube adapter may remove its known empty search-grid shelf, including the heading and Show more controls. Require that only plain empty layout wrappers remain; preserve hidden/new cards, unknown content, private controls, open shadow roots and initially empty/loading shelves. This local layout cleanup adds no judgment, removal count or history record.
...
The typical layout-preserving removal commits around 1150ms after judgment. A 1.8-second fallback handles stalled animation completion. Hidden tabs, offscreen or already-hovered targets, disabled animation, and reduced-motion preferences use immediate removal. Scrolling during an active removal commits it immediately through the same freshness guard, before page scroll handlers can change hover/classes and restart the animation; document and nested-container scrolling are both handled, including inside open shadow roots. Resizing discards stale decorative geometry without canceling a valid deletion. After release, shards follow document scrolling so automatic scroll anchoring during layout collapse does not erase the explosion. At most four particle layers and four glints run concurrently; additional removals still proceed without queuing. Glints never enter the target’s content or evidence, disappear when the spin ends or on cancellation, and are omitted for targets with an existing transform that cannot be mirrored faithfully. Atlas and output bitmaps are capped near one and two million pixels each. Preserve host inline styles and unrelated animations.
