...
| --- | --- | --- |
| YouTube | Video-card renderers, including compact/hero search watch cards and Shorts lockup v1/v2; empty Shorts search-grid cleanup; reel player; comment renderer; watch player plus `ytd-watch-metadata` (legacy above-the-fold fallback) | Search/watch boundaries observed, including public `black ops` watch cards and Shorts grids. Other layouts remain fixture-tested. No pixels/audio analysis. |
...
- Shorts search grids (`grid-shelf-view-model`) lose their heading, Show more controls and spacing only after a successful last-card removal leaves plain empty layout divs. Hidden/new cards, unknown content, private controls and open shadow roots prevent cleanup; initially empty/loading shelves remain untouched. This is layout cleanup, not another classification or counted removal. Structural tests cover nested v1/v2/rich-item boundaries and synchronous repopulation; real Jev/browser checks cover mixed shelves and exact counts with animation on/off. On 2026-09-20, an unauthenticated public `black ops` search also cleared two actual shelves after their 5 and 12 Shorts were removed. That observation is not universal YouTube layout or classification-accuracy coverage.
